...
*/
void idt_init(){
  add_vector(0, &divide_err, K_LVL, CODE_TYPE); //NOTE: CODE TYPE=trap gate, doesn't clear intr
  add_vector(1, &debug, K_LVL, CODE_TYPE);
  add_vector(2, &nmi, K_LVL, CODE_TYPE);
  add_vector(3, &breakpoint, U_LVL, DATA_TYPE);
  add_vector(4, &overflow, U_LVL, DATA_TYPE); //DATA_TYPE 0 = interrupt gate, clears interrupts flag (cli) which means intr can't occur in handler
