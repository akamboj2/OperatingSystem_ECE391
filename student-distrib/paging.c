...
void paging_init() {
    ...
    pageDirectory[0] =  pageTable + page;
    pageDirectory[1] = KENREL_PHYS_ADDR + kernelPage; //how does processor know this block only has 1 table?--PSE question
    ...
    asm volatile("MOVL %%CR3, %%edx; ANDL 0xFFFFF000, %%edx; ADDL %0,%%edx; MOVL %%edx, %%CR3" //need double %% for registers single % to specify input/output from C
                    : //no outputs
                    : "r"(pageDirectory&&0xFFFFF000) //inputs (need to mask in this case to preserve rest of CR3)
